...
type FeatureCardProps = FeatureType & {
    delay: number;
};
...
const FeatureCard = ({ icon, title, description, delay = 0 }: FeatureCardProps) => {
    ...
    return (
        <div
            ref={ref}
            className={`backdrop-blur-lg bg-white/5 border border-white/10 rounded-2xl p-8 transition-all duration-700 
        ...
        <section className="py-20" id="features">
            <div className="max-w-7xl mx-auto px-6">
                <div className="text-center mb-16">
                    <h2 className="text-5xl font-bold bg-linear-to-r from-blue-500 via-purple-500 to-cyan-400 bg-clip-text text-transparent mb-6">
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {features.map((feature: FeatureType, idx: number) => {
                        return <FeatureCard key={idx} {...feature} delay={idx * 100} />;
                    })}
                </div>
            </div>
        </section>
